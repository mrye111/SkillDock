...
- [Windows 打包配置（NSIS + capabilities）](https://github.com/mrye111/SkillDock/issues/10) — 标准/离线双 EXE 产出并本机安装验收通过（AC-28）；离线引导待无 WebView2 环境实测

## Not yet specified

- 真实 Agent 加载验证矩阵（AC-31）：需要各工具实测版本与作用域，发版前锁定。
- ~~性能实测与调优~~（已完成：扫描 0.72s/≤5s、同步 27.0s/≤30s 达标，见下方评论；基准工具 examples/perf-bench.rs 可复测）
- AC-29 离线包实测：需无 WebView2 的干净系统（产物已就绪：release/ 下 252.5 MiB）。
- AC-28 干净虚拟机复测 + 升级覆盖安装；正式图标替换占位图。
- 与前端联调已完成一轮（事件名/调用时序已对齐）；后续联调按契约变更记录驱动。
