...
| 前端风格 | 简体中文优先，浅灰底、白色工作区、青绿色主操作；重视表格效率与明确反馈 |
...
### 9.1 视觉方向：安静、清晰、像一件顺手的工具

采用符合 Windows 桌面习惯的布局：稳定的左侧导航、顶部技能库选择器、中间高信息密度列表、右侧详情抽屉和底部批量操作栏。内容清晰优先，品牌感来自色彩与排版。

| 设计项 | 规格 |
| --- | --- |
| 默认主题 | 明亮主题；P1 增加暗色及跟随系统 |
| 背景 / 面板 | `#F5F7F8` / `#FFFFFF` |
| 正文 / 次要文字 | `#172B36` / `#546772` |
| 主色 | `#0F766E`，用于主要按钮、选中态和焦点 |
| 分隔线 | `#DFE7EA` |
| 成功 / 提醒 / 失败 | 深绿 / 深琥珀 / 深红，必须配合图标和状态文字 |
| 字体 | Segoe UI Variable、Microsoft YaHei UI；路径和差异使用 Cascadia Mono / Consolas |
| 字号 | 页面标题 24 px，分组标题 16 px，正文 14 px，辅助文字 12 px |
| 尺寸 | 8 px 间距基准，面板圆角 12 px，控件圆角 8 px，列表行高 60 px |
| 动效 | 抽屉与状态切换 120–180 ms；尊重系统减少动画设置 |
| 窗口 | 默认 1280 × 820 DIP，最小 960 × 640 DIP；窄窗口收拢导航，矩阵横向滚动 |
...
P1 的自动同步和方案管理作为现有页面的二级功能，不增加首屏导航负担。

### 9.3 主工作台线框

以下为示例状态；图中的工具列代表已配置目标，同一物理目录会合并展示关联工具。

```text
┌──────────────────────────────────────────────────────────────────────────────┐
│ SkillDock           我的 Skills ▾       C:\Work\my-skills       刷新    ─ □ × │
├────────────┬─────────────────────────────────────────────────────────────────┤
│            │ 技能库                                        添加技能库       │
│ ● 技能库   │ 24 个技能 · 6 项待更新 · 1 项需处理                            │
│   同步目标 │                                                                 │
│   同步历史 │ 搜索名称、描述或目录…              全部 ▾   仅看待同步 □       │
│            ├──────────────────────┬─────────┬─────────┬─────────┬─────────────┤
│            │ 技能                 │ Codex   │ Claude  │ Cursor  │ Copilot     │
│            │ □ code-review        │ 已同步  │ 待更新  │ 待更新  │ 未选择      │
│            │   检查代码与潜在问题 │         │         │         │             │
│            │ □ meeting-notes      │ 待新增  │ 待新增  │ 未选择  │ 已同步      │
│            │   整理会议行动项     │         │         │         │             │
│            │ □ release-helper     │ 已同步  │ 需处理  │ 未选择  │ 未选择      │
│            ├──────────────────────┴─────────┴─────────┴─────────┴─────────────┤
│ 设置       │ 已选 2 个技能     目标：Codex、Claude Code ▾    [预览同步 →]   │
└────────────┴─────────────────────────────────────────────────────────────────┘
```

点技能名称打开详情；点目标状态打开该映射详情；悬浮显示完整路径。未勾选技能时底栏说明“勾选技能后开始同步”，不显示难理解的禁用按钮堆叠。

### 9.4 重点页面

| 页面 | 主要内容 | 主操作 |
| --- | --- | --- |
| 技能库 | 列表、矩阵、搜索筛选、源库状态 | 预览同步 |
| 目标页 | 工具卡片、用户/项目作用域、目录来源、可用性 | 添加目标 |
| 预览抽屉 | 分目标的操作清单、数量、差异、冲突 | 同步 N 项 |
| 冲突抽屉 | 源/目标/基线差异、覆盖范围、备份说明 | 备份后用源覆盖 |
| 任务结果 | 成功、失败、跳过、取消、待恢复及重试入口 | 重试失败项 |
| 历史页 | 时间线或表格、任务详情、备份是否可用 | 恢复所选目标 |

### 9.5 文案与反馈

| 场景 | 展示示例 |
| --- | --- |
| 空技能库 | 这里还没有找到 Skills。请选择包含技能文件夹的目录。 |
| 没有目标 | 选择要同步到的工具，应用会显示它的技能目录。 |
| 已一致 | 所选技能在这些目标中已是最新状态。 |
| 冲突 | Claude Code 中的 code-review 有本地修改。查看差异后选择保留或覆盖。 |
| 部分完成 | 已完成 11 项，1 项失败，2 项仍需处理。 |
| 权限失败 | 无法写入此目录。请选择有写入权限的位置，或调整文件夹权限后重试。 |
| 计划失效 | 文件在预览后发生了变化，请刷新预览。 |
| 加载提示 | 文件已同步。如工具尚未显示，请按该工具的说明刷新或开启新会话。 |

错误信息必须包含受影响技能、目标路径、实际原因和可执行的下一步；技术堆栈折叠在“诊断详情”。成功反馈使用页面状态和轻提示，避免每次弹出确认框。

### 9.6 键盘和可访问性

`Ctrl+O` 打开技能库，`Ctrl+F` 聚焦搜索，`F5` 刷新，`Ctrl+Enter` 在主列表打开同步预览，`Esc` 关闭抽屉。执行中的取消有独立按钮，Esc 不触发文件操作。预览中的提交仅通过明确按钮或按钮获得焦点后的 Enter 触发。

所有交互支持键盘，焦点可见，抽屉关闭后焦点回到来源。状态具有文字和辅助技术名称。适配 Windows 100%、150%、200% 缩放与高对比度，长路径可展开、复制和横向滚动，错误说明不能仅靠悬浮提示读取。
...
│   ├── capabilities/
│   └── tauri.conf.json
├── tests/fixtures/          # 隔离的源、目标、冲突和恢复样本
└── docs/
```
...
- 默认当前用户安装到 `%LOCALAPPDATA%/Programs/SkillDock`，创建开始菜单入口，桌面快捷方式可选。
...
在 Windows 构建机上固定 Node、包管理器、Rust 工具链及依赖锁文件，执行前端检查、Rust 检查、核心测试、前端构建，再执行 Tauri 的 Windows NSIS 打包。标准版和离线版使用独立构建配置，共享应用版本和代码。
